    ...
    <tbody>
        @if ($users->count() > 0)
            ...
            <tr>
                ...
                <td>
                    @can('view_user')
                        <a href="{{route('admin.customer.show', $user->id)}}" class="btn btn-sm btn-clean btn-icon" title="view">
                            <i class="la la-eye"></i>
                        </a>
                    @endcan

                    @can('edit_user')
                        <a href="{{route('admin.customer.edit', $user->id)}}" class="btn btn-sm btn-clean btn-icon" title="Edit">
...
@push('stackScript')
    <script> 
        $(".block").click(function(e) {
            ...
            var url =  '<a href="{{route("admin.customer.block",":id")}}" class="swal2-confirm swal2-styled" title="Block">Confirm</a>';
    ...
    <script> 
        $(".delete").click(function(e) {
            ...
            var data_id = $(this).attr("data-id");
            var url =  '<a href="{{route("admin.customer.delete",":id")}}" class="swal2-confirm swal2-styled" title="Delete">Confirm</a>';
            url = url.replace(':id', data_id );
        ...
        $(".approve").click(function(e) {
            ...
            var data_id = $(this).attr("data-id");
            var url =  '<a href="{{ route("admin.customer.approve", ":id") }}" class="swal2-confirm swal2-styled" title="Approve">Approve</a>';
            url = url.replace(':id', data_id );

            Swal.fire({
                title: 'Are you sure want to approve this customer?',
                icon: 'info',
                showCancelButton: true,
                confirmButtonText: url,
            }).then((result) => {
                if (result.isConfirmed) {
                    Swal.fire('Customer Approved Successfully!', '', 'success')
                } else if (result.dismiss === "cancel") {
                    Swal.fire('Canceled', '', 'error')
                }
            })
